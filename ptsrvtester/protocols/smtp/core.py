...
class SMTP(ReconMixin, ProtocolMixin, AuthMixin, EnumMixin, RelayMixin, DeliveryMixin, ContentMixin, StressMixin, IndirectMixin, ReportingMixin, ConnectionMixin, SharedMixin, BaseModule):
    ...
    # Indirect tests run before standalone direct paths and must not block them.
    _INDIRECT_TEST_DESTS = frozenset({"blacklist_test", "spf_test"})

    def _active_direct_test_dests(self) -> frozenset[str]:
        """Active direct-scan test dests from the registry (+ brute).

        Derived from ``SMTP_TEST_DESTS`` / ``SMTP_VALUE_DESTS`` so a new test
        automatically participates in exclusivity checks. Indirect tests
        (blacklist/SPF) are excluded — they run earlier and may combine with
        a standalone direct test.
        """
        active: set[str] = set()
        for dest in SMTP_TEST_DESTS:
            if dest in self._INDIRECT_TEST_DESTS:
                continue
            if getattr(self.args, dest, False):
                active.add(dest)
        for dest in SMTP_VALUE_DESTS:
            if getattr(self.args, dest, None) is not None:
                active.add(dest)
        if self.do_brute:
            active.add("brute")
        return frozenset(active)

    def _is_only_tests(self, *allowed: str) -> bool:
        """True when the set of active direct tests is non-empty and ⊆ ``allowed``."""
        active = self._active_direct_test_dests()
        allowed_set = frozenset(allowed)
        return bool(active) and active <= allowed_set
    ...
    def run(self):
        ...
        if self._is_only_tests("invalid_commands"):
        ...
        if self._is_only_tests("helo_only"):
        ...
        if self._is_only_tests("helo_bypass"):
        ...
        if self._is_only_tests("identify", "id_aggressive"):
        ...
        if self._is_only_tests("bounce_replay"):
        ...
        # BOMB / AV / SSRF / FLOOD / ZIPXXE (any combination; fixed order)
        if self._is_only_tests("bomb", "antivirus", "ssrf", "flood", "zipxxe"):
        ...
        if self._is_only_tests("spoof_headers"):
        ...
        if self._is_only_tests("bcc_test"):
        ...
        if self._is_only_tests("alias_test"):
        ...
        # Only ENCRYPT: test_encryption() opens its own connections
        if self._is_only_tests("isencrypt"):
        ...
        if self._is_only_tests("role_identify"):
        ...
        if self._is_only_tests("enumerate"):
        ...
        if self._is_only_tests("commands", "authentications"):
            try:
                smtp, info = self.initial_info(get_commands=True)
                self.results.info = InfoResult(
                    None,
        ...
        if self._is_only_tests("rcpt_limit"):
        ...
        if self._is_only_tests("rcpt_duplicate"):
        ...
        if self._is_only_tests("probe_accepted_domain"):
        ...
        if self._is_only_tests("interactive") and not self.use_json:
            try:
        ...
        if self._is_only_tests("open_relay"):
        ...
        if self._is_only_tests("rate_limit"):
        ...
        if self._is_only_tests("noop_flood1"):
            self.ptprint(
        ...
        if self._is_only_tests("noop_flood2"):
        ...
        if self._is_only_tests("auth_format"):
        ...
        if self._is_only_tests("auth_enum"):
        ...
        if self._is_only_tests("helo_validation"):
        ...
        if self._is_only_tests("auth_downgrade"):
