...
from .helpers import Target, ArgsWithBruteforce, add_bruteforce_args
...
def _enum_method_choice(value: str) -> str:
    """``-e`` method, case-insensitive. ``ALL`` keeps VRFY, EXPN and RCPT."""
    method = (value or "").strip().upper()
    allowed = ("VRFY", "EXPN", "RCPT", "ALL")
    if method not in allowed:
        raise argparse.ArgumentTypeError(
            f"invalid choice: {value!r} (choose from {', '.join(allowed)})"
        )
    return method
...
class SMTPArgs(ArgsWithBruteforce):
  ...
  ptsrvtester smtp -ts BOUNCE -m attacker@example.com -r foo@foo.com -tg smtp.example.com:25
  ptsrvtester smtp -ts BOMB -r victim@example.com -tg smtp.example.com:587
  ptsrvtester smtp -ts AV -r victim@example.com -tg smtp.example.com:587
  ptsrvtester smtp -ts SSRF -r victim@example.com --ssrf-canary-url https://xyz.oast.fun/ssrf -tg smtp.example.com:587
  ptsrvtester smtp -ts AV,SSRF,ZIPXXE -r victim@example.com --ssrf-canary-url http://cb --zipxxe-canary-url http://cb -tg smtp.example.com:25
  ptsrvtester smtp -ts SPOOF -r victim@example.com -u user -p pass -tg smtp.example.com:587
  ptsrvtester smtp -ts BCC -r to@example.com --cc cc@example.com --bcc bcc@example.com -tg smtp.example.com:25
  ptsrvtester smtp -ts AV -h"""
        ...
        direct.add_argument(
            ...
            nargs="?",
            type=int,
            const=NOOP2_DEFAULT_CONNECTIONS,
            default=None,
            metavar="N",
            dest="noop2_count",
            help=argparse.SUPPRESS,
        ...
        direct.add_argument(
            ...
            metavar="domain",
            default=None,
            help="Mail domain when a test needs one (default: from server banner/EHLO)",
        )
        direct.add_argument(
            "-R",
            "--role",
