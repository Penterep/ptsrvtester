...
SMTP_TESTS: dict[str, dict] = {
    ...
    "IDENTIFY": {
        ...
        "flags": {"identify": True},
        "mods": [
            ["-d", "--domain", "<domain>", "Mail domain for MX vs certificate (default: banner/EHLO)"],
        ],
    },
    "IDAGG": {
    ...
    "ENUM": {
        ...
        "mods": [
            ...
            ["-d", "--domain", "<domain>", "Recipient domain for RCPT (default: banner/EHLO)"],
            ["-u", "--user", "<name> …", "Candidate username(s)"],
            ["-U", "--users", "<wordlist>", "Username wordlist"],
    ...
    "NOOP1": {
        ...
        "long": ["Test how long a connection can be maintained with periodic NOOP.",
                 "RFC 5321 §4.1.1.9: NOOP must return 250."],
        "flags": {"noop_flood1": True},
        "mods": [
            ["", "--duration", "<sec>", "How long the test runs (default: 35 min)"],
            ["", "--delay", "<sec>", "Wait between NOOPs (default: 4 min; 0 = max speed)"],
        ],
    },
    "NOOP2": {
        "desc": "NOOP connection count",
        "long": ["Test how many connections can be established and maintained with",
                 "periodic NOOP. Evaluates per-IP connection limits."],
        "value": ("noop2_count", NOOP2_DEFAULT_CONNECTIONS),
        "mods": [
            ["", "--count", "<n>", f"Max connections to attempt (default: {NOOP2_DEFAULT_CONNECTIONS})"],
            ["", "--duration", "<sec>", "How long the test runs (default: 120s)"],
            ["", "--delay", "<sec>", "Wait between NOOPs (default: 60s; 0 = max speed)"],
            ["-t", "--threads", "<n>", "Parallel connect threads (default: 1)"],
            ["-nf2", "--noop-flood2", "<n>", "Alias for --count"],
        ],
    },
    "BOMB": {
...
def _smtp_test_help(codes: list[str]):
    ...
    for code in valid:
        ...
        usage = f"ptsrvtester smtp -ts {code} " + ("<options> -tg <target>" if has_opts else "-tg <target>")
