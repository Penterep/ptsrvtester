...
class ReportingMixin:
        ...
        if accepted:
            for line in reversed(smtp_trace):
                if line.startswith("DATA:"):
                    code = SharedMixin._data_trace_status_code(line)
                    if code:
    ...
    def _collect_flat_vulns(self) -> list[dict]:
        ...
        if (enc := self.results.encryption) is not None:
            any_inc = (
                getattr(enc, "plaintext_incomplete", False)
                or getattr(enc, "starttls_incomplete", False)
                or getattr(enc, "tls_incomplete", False)
            )
            if enc.plaintext_ok and not enc.starttls_ok and not enc.tls_ok and not any_inc:
    ...
    def output(self) -> None:
        ...
        if self._is_enum_only_output():
            ...
            if (catch_all := self.results.catch_all) is not None:
                desc_map = {
                    ...
                    "unreachable": "CatchAll timed out (not confirmed)",
                }
            ...
            if enum_results is not None:
                ...
                requested_set = enum_methods_from_arg(self.args.enumerate)
        ...
        if (ntlm_error := self.results.ntlm_error) is not None:
            ...
            if getattr(ntlm, "incomplete", False):
                properties.update({"ntlmInfoStatus": "incomplete"})
            elif not ntlm.success:
